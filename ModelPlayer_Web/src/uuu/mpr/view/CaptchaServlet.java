 ...
 */
//@WebServlet(urlPatterns = "/images/captcha.jpg") //這一行改到web.xml中設定
public class CaptchaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String captcha = "";
		for(int i = 0 ; i < len ; i++) {
			int data = random.nextInt(36); //用亂數取得0~36(不含36)之間的一個變數
			char ch = (char)(data < 10 ? (data + '0'):(data-10+'A'));
			captcha += ch;
		}
		
		System.out.println(captcha);	//測試用，會將captcha字串出在server consle
